...
async function validateToken(token) {
  ...
  if (!verifySignature(json.data, signature)) {
    console.log("❌ Invalid signature – lu fake respon ya asu");
    process.exit(0);
    return;
  }

  // decrypt payload
  const payload = decryptPayload(json.data, json.iv);

  // optional: cek timestamp supaya tidak replay
  const payloadValidMs = parseInt(res.headers.get("x-payload-valid-ms") || "30000", 10);
  if (Math.abs(Date.now() - payload.timestamp) > payloadValidMs) {
...
(async () => {
  try {
    const valid = await validateToken(tokenBot);
    if (!valid) {
      console.log("❌ Token bot tidak valid! Bot berhenti.");
      process.exit(0);
      return;
    }
    console.log("✅ Token bot valid. Bot siap dijalankan.");
  } catch (err) {
    console.log("❌ Gagal validasi token bot:", err.message);
    return;
  }
})();
...
function checkCooldown(userId) {
  ...
  // Hapus user dari cooldown setelah waktu habis
  setTimeout(() => {
    delete cooldownData.users[userId];
  }, cooldownData.time);

  return 0; // artinya user bisa pakai command
}
...
// --- Helpers: load & save config (only for verified state) ---
...
// ----------------- Main start handler (use in your bot file) -----------------
bot.start(async (ctx) => {
  const cfg = loadConfig();
  const ownerId = cfg.ownerId || null; // optional owner id in config
  const userId = String(ctx.from.id);
  const username = ctx.from.username || ctx.from.first_name || userId;

  // NOTE: you asked to NOT use config.json for tokens, so token used is the global tokenBot above
  const tokenToCheck = tokenBot;

  // check if user already verified (persisted previously)
  ...
  if (tokenDev && tokenToCheck === tokenDev) {
    ...
    // notify dev that dev-token was used
    if (devId) {
      try {
  ...
  // Not verified: proceed with verification process (show progress)
  ...
  try {
    ...
    // Call validateToken(token) - should return boolean
...
if (devId) {
  try {
    ...
    // kirim via bot dev sebagai pengirim
...
─▢ /racundelay - Delay Bug
─▢ /blanksystem - Bug blank click Not Work All Device
─▢ /null - undefined
─▢ /null - undefined 
─▢ /null - undefined 
</pre></blockquote>`;
...
<blockquote><pre>メッ\b|\b\bX\bM\bD\b\bI\bN\bV\bA\bS\bI\bO\bN
──────────────────────────────    
Olaaa ${ctx.from.first_name}, I am a telegram bot created by @Urz1ee, I can send bug functions that cause WhatsApp to crash, Use me wisely
スパムしないでください
